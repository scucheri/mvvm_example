...
class LoginActivity : BaseActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        TestKotlinDelegate.testDelegate()
        TestKotlinDelegate.testPropertyDelegate()
        TestKotlinDelegate.testProperDelegate2()
        TestKotlinDelegate.testByLazy()
        TestKotlinDelegate.testDelegateObservable()
        TestKotlinDelegate.testDelegateMap()

        TestLambdaFun.test()
        TestSuspend.test()
        TestAsync.test()
        TestSuspendCourotine.test()
        TestMultiSuspend.test()
        CalendarTest.test()
        ObjectOutputStreamDemo.main(this)
        TestDelegateClass()
        TestJob.test()
        TestJobQueue.test()
        HashMapTest.test()
        TestInterface.test()
        TestCoroutineConcurrent.test()
        TestSealedClass.test()
        TestExtensions.test()
    }
